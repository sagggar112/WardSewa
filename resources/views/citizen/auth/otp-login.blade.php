...
<div class="min-h-[70vh] flex items-center justify-center px-4 py-12">
    <div class="max-w-md w-full bg-white rounded-2xl border border-slate-200 shadow-lg p-8">
        <div class="text-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-slate-900 mt-4">{{ __('SMS OTP लगइन (Login via OTP)') }}</h2>
            <p class="text-xs text-slate-500 mt-1">{{ __('Enter your 10-digit Nepali mobile number to receive a one-time login code.') }}</p>
        </div>
        ...
        <div class="mt-6 pt-5 border-t border-slate-100 text-center text-xs text-slate-500">
            <a href="{{ route('citizen.login') }}" class="text-nepal-blue font-bold hover:underline">
                &larr; {{ __('पासवर्ड मार्फत लगइन गर्नुहोस् (Login with Password)') }}
            </a>
        </div>
    </div>
</div>
